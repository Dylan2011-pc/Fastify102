...
closeOnClick/>
   <div className="main-content">
        ...
        <Col xs={2.5} className="inputsearchs">
                      <Form.Control  className='inputsearch' placeholder="Tìm kiếm thiết bị" onChange = {(e)=>setnamesearch(e.target.value)} />
     ...
     <option value={nhom.id} key={index}>{nhom.Name}</option>
                      ...
                      <Form.Group className='fngayd'>
        <Form.Label >Ngày duyệt : </Form.Label>
         
    <DatePicker className='ngayd' value={datetra} onChange={getTimemuon} />
     </Form.Group>


     <Form.Group className = "fngayt">
        <Form.Label  >Ngày Trả : </Form.Label>
   
        <DatePicker className='ngayt' value={datetra} onChange={getTimemuon} />
     </Form.Group>


                    <button type="button" className="btn btn-success buttonsearch marginToplist" onClick={()=>handleSearchDevice()}> Tìm
